...
if config_env() == :prod do
  ...
  # Parse Redis URL into host/port/password components
  ...
  redis_password = if redis_uri.userinfo do
    case String.split(redis_uri.userinfo, ":") do
      [_, pass] -> pass
      _ -> nil
    end
  end
  ...
  redis_opts = if redis_password, do: Keyword.put(redis_opts, :password, redis_password), else: redis_opts
  redis_opts = if System.get_env("REDIS_SSL", "false") == "true", do: Keyword.put(redis_opts, :ssl, true), else: redis_opts

  config :quckapp_realtime, :redis, redis_opts

  # Redis PubSub for Phoenix channels
  config :quckapp_realtime, QuckAppRealtimeWeb.Endpoint,
